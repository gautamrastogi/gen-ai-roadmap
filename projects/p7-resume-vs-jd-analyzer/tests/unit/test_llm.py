...
def test_prompt_mode_uses_chat_without_response_format() -> None:
    ...
    client = _mock_chat_client('{"fit_score": 80}')
...
def test_chat_schema_mode_sends_json_schema_response_format() -> None:
    ...
    assert response_format["json_schema"]["name"] == "resume_jd_analysis"
    assert response_format["json_schema"]["strict"] is True


def test_responses_schema_mode_sends_text_format() -> None:
    settings = Settings(
        analysis_mode="responses_schema",
        openai_base_url="",
        openai_api_key=SecretStr("sk-test"),
        openai_model="gpt-test",
    )
    response = SimpleNamespace(
        output_text='{"fit_score": 80}',
        model="gpt-test",
    ...
    assert result.text == '{"fit_score": 80}'
    assert text_format["type"] == "json_schema"
    assert text_format["name"] == "resume_jd_analysis"
