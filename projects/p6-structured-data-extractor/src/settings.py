"""Runtime settings for P6 structured extraction."""
...
class Settings(pydantic_settings.BaseSettings):
    ...
    extraction_mode: schemas.ExtractionMode = pydantic.Field(
        default="prompt",
        validation_alias=pydantic.AliasChoices("EXTRACTION_MODE", "MODE"),
    )
    ...
    max_tokens: int = pydantic.Field(default=1200, ge=1, le=20_000)
    max_input_tokens: int = pydantic.Field(default=8000, ge=1, le=200_000)
    ...
    def provider(self) -> ProviderName:
        ...
        if self.extraction_mode == "responses_schema":
    ...
    def validate_provider(self) -> "Settings":
        """Ensure modes have the credentials they need."""

        has_openai_key = bool(self.openai_api_key.get_secret_value().strip())
        if self.extraction_mode == "responses_schema" and not has_openai_key:
            raise ValueError("OPENAI_API_KEY is required when EXTRACTION_MODE=responses_schema.")
        if (
            self.extraction_mode in {"prompt", "chat_schema"}
